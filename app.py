...
# Corrigido: postgreql -> postgresql
...
def query_db(query, args=(), one=False):
    db = get_db()
    cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)  # Retorna dicionários
...
def cadastro_produto():
    if request.method == 'POST':
        ...
        if produto:
            ...
            produto_id = execute_db(
                'INSERT INTO produtos (nome, descricao, quantidade, preco, quantidade_minima) VALUES (%s, %s, %s, %s, %s) RETURNING id', 
    ...
    # Corrigido: use o nome completo das colunas para evitar erro
    produtos = query_db(
